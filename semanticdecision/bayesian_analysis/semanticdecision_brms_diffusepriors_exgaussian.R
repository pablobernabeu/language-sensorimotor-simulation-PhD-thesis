...
# Model with diffuse priors (SD = 0.3)
...
diffuse_priors =
  c(set_prior('normal(0, 0.3)', class = 'Intercept'),
    ...
    set_prior('normal(0, 0.3)', class = 'sd'),  # automatically truncated to keep positive values only
    set_prior('normal(0, 0.3)', class = 'sigma'),  # automatically truncated to keep positive values only
    set_prior('lkj(2)', class = 'cor')  # standard, regularising prior on random-effects covariance to aid convergence
  )

# Model

semanticdecision_brms_diffusepriors_exgaussian =
  ...
  brm(
    ...
    data = semanticdecision, prior = diffuse_priors,
...
# saveRDS(semanticdecision_brms_diffusepriors_exgaussian,
#         'semanticdecision/bayesian_analysis/results/semanticdecision_brms_diffusepriors_exgaussian.rds')

# Model summary
summary(semanticdecision_brms_diffusepriors_exgaussian) %>%
  saveRDS('semanticdecision/bayesian_analysis/results/semanticdecision_summary_diffusepriors_exgaussian.rds')
...
pp_check(semanticdecision_brms_diffusepriors_exgaussian, ndraws = 100) %>%
  saveRDS('semanticdecision/bayesian_analysis/posterior_predictive_checks/semanticdecision_posteriorpredictivecheck_diffusepriors_exgaussian.rds') 

# Plot posterior distribution of each fixed effect (i.e., effects commencing with 'b_')
mcmc_plot(semanticdecision_brms_diffusepriors_exgaussian, 
          type = 'areas', variable = '^b_', regex = TRUE) %>%
  saveRDS('semanticdecision/bayesian_analysis/results/posteriordistributions_semanticdecision_diffusepriors_exgaussian.rds')

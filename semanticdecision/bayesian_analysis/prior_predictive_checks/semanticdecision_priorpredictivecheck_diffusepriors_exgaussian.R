...
# PRIOR PREDICTIVE CHECK WITH DIFFUSE PRIORS
...
sampleonly_semanticdecision_brms_diffusepriors =
...
p1 = pp_check(sampleonly_semanticdecision_brms_diffusepriors,
              type = 'stat', stat = 'min') +
  xlab('Response time (*z*)') +
  ggtitle('Prior predictive distribution of minimum values') +
  theme(axis.title.x = ggtext::element_markdown())

p2 = pp_check(sampleonly_semanticdecision_brms_diffusepriors,
              type = 'stat', stat = 'mean') +
  xlab('Response time (*z*)') +
  ggtitle('Prior predictive distribution of means') +
  theme(axis.title.x = ggtext::element_markdown())

p3 = pp_check(sampleonly_semanticdecision_brms_diffusepriors,
              type = 'stat', stat = 'max') +
  xlab('Response time (*z*)') +
...
# Free up workspace
# rm(sampleonly_semanticdecision_brms_diffusepriors)

p123 = p1 + p2 + p3 + plot_layout(ncol = 1)

ggsave(p123,
       filename = 'semanticdecision/bayesian_analysis/prior_predictive_checks/plots/semanticdecision_priorpredictivecheck_diffusepriors_exgaussian.pdf',

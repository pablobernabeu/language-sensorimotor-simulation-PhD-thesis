...
sampleonly_lexicaldecision_brms_weaklyinformativepriors =
...
p1 = pp_check(sampleonly_lexicaldecision_brms_weaklyinformativepriors,
              type = 'stat', stat = 'min') +
  xlab('Response time (*z*)') +
  ggtitle('Prior predictive distribution of minimum values') +
  theme(axis.title.x = ggtext::element_markdown())

p2 = pp_check(sampleonly_lexicaldecision_brms_weaklyinformativepriors,
              type = 'stat', stat = 'mean') +
  xlab('Response time (*z*)') +
  ggtitle('Prior predictive distribution of means') +
  theme(axis.title.x = ggtext::element_markdown())

p3 = pp_check(sampleonly_lexicaldecision_brms_weaklyinformativepriors,
              type = 'stat', stat = 'max') +
  xlab('Response time (*z*)') +
...
# Free up workspace
# rm(sampleonly_lexicaldecision_brms_weaklyinformativepriors)

p123 = p1 + p2 + p3 + plot_layout(ncol = 1)

ggsave(p123,
       filename = 'lexicaldecision/bayesian_analysis/prior_predictive_checks/plots/lexicaldecision_priorpredictivecheck_weaklyinformativepriors_exgaussian.pdf',

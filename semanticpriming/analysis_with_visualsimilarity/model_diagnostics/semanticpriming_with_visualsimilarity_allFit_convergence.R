...
semanticpriming_with_visualsimilarity_allFit_convergence =
  allFit(semanticpriming_with_visualsimilarity_lmerTest, 
...
# Save (not saved because it takes up 400 MB)
# saveRDS(semanticpriming_with_visualsimilarity_allFit_convergence,
#         'semanticpriming/analysis_with_visualsimilarity/model_diagnostics/results/semanticpriming_with_visualsimilarity_allFit_convergence.rds')
...
labels = 
  colnames(semanticpriming_with_visualsimilarity_allFit_convergence[[1]]@pp$X) %>% 
  ...
  # the word-level variables of interest (i.e., 'z_cosine_similarity', 
  # 'z_visual_rating_diff' and 'z_visual_similarity') sometimes appeared second 
  # in their interactions. For better consistency, the code below moves those 
  # word-level variables (with their new names) to the first position in their 
  # interactions. Note that the order does not affect the results in any way.
...
for(i in 1 : length(semanticpriming_with_visualsimilarity_allFit_convergence)) {
  X_temp = semanticpriming_with_visualsimilarity_allFit_convergence[[i]]@pp$X
  colnames(X_temp) = labels # <-- labels created above
  semanticpriming_with_visualsimilarity_allFit_convergence[[i]]@pp =
    merPredD(X = X_temp, 
             Zt = semanticpriming_with_visualsimilarity_allFit_convergence[[i]]@pp$Zt, 
             Lambdat = semanticpriming_with_visualsimilarity_allFit_convergence[[i]]@pp$Lambdat, 
             Lind = semanticpriming_with_visualsimilarity_allFit_convergence[[i]]@pp$Lind, 
             theta = semanticpriming_with_visualsimilarity_allFit_convergence[[i]]@pp$theta, 
             n = nrow(semanticpriming_with_visualsimilarity_allFit_convergence[[i]]@pp$X))
}
...
# Main effects
( plot.fixef.allFit(semanticpriming_with_visualsimilarity_allFit_convergence, 
  ...
  # Save plot
  ggsave(filename = 'semanticpriming/analysis_with_visualsimilarity/model_diagnostics/plots/main_effects_semanticpriming_with_visualsimilarity_allFit_convergence.pdf', 
         device = cairo_pdf, width = 9, height = 10.5, dpi = 900)

# Interactions (not all interactions plotted due to the size of the plot)
( plot.fixef.allFit(semanticpriming_with_visualsimilarity_allFit_convergence, 
                    ...
                    multiply_y_axis_limits = 1.3) ) %>%
  # Save plot
  ggsave(filename = 'semanticpriming/analysis_with_visualsimilarity/model_diagnostics/plots/interactions_semanticpriming_with_visualsimilarity_allFit_convergence.pdf', 
         device = cairo_pdf, width = 9, height = 11.6, dpi = 900)

# Free up some memory
rm(semanticpriming_with_visualsimilarity_allFit_convergence)
